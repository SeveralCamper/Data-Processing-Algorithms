...
phonebooknode_t PhoneBookAgeSearch(phonebook_t *phone_book, int age) {
  ...
  int left = 0, middle = 0,
      right = phone_book->phone_book_[phone_book->length_ - 1].index_;
  while (left <= right) {
    middle = (left + right) / 2;
...
void PhoneBookPopBack(phonebook_t *phone_book) {
  ...
  for (int i = 0; i < phone_book->length_; i++) {
    ...
    for (int j = 0; j < 11; j++) {
      phone_book_tmp[i].phone_numbers_[j] =
          phone_book->phone_book_[i].phone_numbers_[j];
    }

    for (int j = 0; j < 25; j++) {
      phone_book_tmp[i].owner_name_[j] =
          phone_book->phone_book_[i].owner_name_[j];
    }
  }
...
void InitPhoneBook(phonebook_t *phone_book, int length) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    phone_book->phone_book_[i] =
        InitPhoneBookNode(i, random(60), new_telephone_num, new_name);
  }
}
...
void SelectSortAge(phonebook_t *phone_book) {
  ...
  for (int i = 0; i < phone_book->length_ - 1; i++) {
    ...
    for (int j = i + 1; j < phone_book->length_; j++) {
      if (phone_book->phone_book_[j].age_ <
          phone_book->phone_book_[minArrayIndex].age_) {
        minArrayIndex = j;
      }
    }
...
void SelectSortIndex(phonebook_t *phone_book) {
  ...
  for (int i = 0; i < phone_book->length_ - 1; i++) {
    minArrayIndex = i;
    for (int j = i + 1; j < phone_book->length_; j++) {
      if (phone_book->phone_book_[j].index_ <
          phone_book->phone_book_[minArrayIndex].index_) {
        minArrayIndex = j;
      }
    }
    phonebooknode_t tmp_book = phone_book->phone_book_[minArrayIndex];
